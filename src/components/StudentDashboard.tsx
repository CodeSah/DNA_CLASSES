    ...
    setTimeout(() => {
      try {
        if (iframe.contentWindow) {
          ...
          setPrintStatusText('Print signal dispatched successfully! Doing hidden frame rendering to keep it on exactly one page.');
        } else {
          throw new Error('No contentWindow available');
        }
      } catch (printErr) {
        console.warn('Sandbox blocked direct iframe printing.', printErr);
        setPrintStatusText('Print layout compiled. Click the "Open in New Tab" button in the upper right corner to avoid browser iframe restrictions.');
      }
    }, 250);
  };
